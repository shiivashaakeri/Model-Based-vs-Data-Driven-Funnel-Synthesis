...
def traj_gen(x_traj, u_traj, Q_traj, K_traj, main_iter) -> tuple:
    max_iter = 20 if main_iter == 0 else 20  # noqa: RUF034
    W_traj = np.zeros([T - 1, ct.nw])
    subproblem_cost_old = 0
    for iter in range(max_iter):  # noqa: A001
        [A_list, B_list, F_list] = jax.vmap(
            lambda x, u, W: linearization_jit(Integrator.RK4, dt, x, u, W), in_axes=(0, 0, 0)
...
def plotting_fcn(x_traj, u_traj, Q_traj):  # noqa: ARG001
    fig, ax = plt.subplots()
    for t in range(T - 1):
        theta = x_traj[t, 2]
        leng = 0.4
        heading_vector = np.array(
            [[x_traj[t, 0], x_traj[t, 1]], [x_traj[t, 0] + leng * np.cos(theta), x_traj[t, 1] + leng * np.sin(theta)]]
        )
