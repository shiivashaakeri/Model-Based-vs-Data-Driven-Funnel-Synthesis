...
def solve_subproblem(A_list, B_list, trajs, x_des) -> tuple:
    ...
    for t in range(T - 1):
        ...
        ## input constraints
        constraints.append(cp.abs(u_t[0] + w_t[0]) <= mass * ct.g * 1.5)
        constraints.append(cp.abs(u_t[1] + w_t[1]) <= 0.00255)
        constraints.append(cp.abs(u_t[2] + w_t[2]) <= 0.00255)
        constraints.append(cp.abs(u_t[3] + w_t[3]) <= 0.00455)

        ## ground constraint
        constraints.append(x_t[2] + d_t[2] <= 0)
        # constraints.append()
...
def traj_gen(x_traj, u_traj, Q_traj, K_traj, main_iter) -> tuple:
    ...
    for iter in range(max_iter):
        ...
        if np.abs(subproblem_cost_old - subproblem_cost) <= 1 and iter > 20:
            break

        if np.abs(subproblem_cost_old - subproblem_cost) <= 0.1 or iter > 15:
            plotting3d_fcn(x_traj)
