...
def solve_subproblem(A_list, B_list, trajs, x_des) -> tuple:
    ...
    for t in range(T - 1):
        ...
        ## for the linear vel
        a_t_1 = np.array([1, 0])
        constraints.append(LA.norm(control_funnel @ a_t_1, 2) + cp.norm(u_t[0] + w_t[0], 1) <= ct.u1_max)
        a_t_2 = np.array([-1, 0])
        # constraints.append(u_t[0] + w_t[0] >= 0)
        constraints.append(LA.norm(control_funnel @ a_t_2, 2) - (u_t[0] + w_t[0]) <= ct.u1_min)
        ## for the omega
        a_t_3 = np.array([0, 1])
        constraints.append(LA.norm(control_funnel @ a_t_3, 2) + cp.norm(u_t[1] + w_t[1], 1) <= ct.u2_max)
...
def traj_gen(x_traj, u_traj, Q_traj, K_traj, main_iter) -> tuple:
    ...
    for iter in range(max_iter):
        ...
        if np.abs(subproblem_cost_old - subproblem_cost) <= 1:
            break
...
def plotting_fcn(x_traj, u_traj, Q_traj):
    ...
    ax.plot(x_traj[0, 0], x_traj[0, 1], "b.", markersize=10, label="Start")
    ax.legend()
    for t in range(T - 1):
        theta = x_traj[t, 2]
        leng = 0.4
